...
client.on("interactionCreate", async (interaction) => {
  ...
  if (command === "ting") {
    ...
    if (!midjourneyChannel)
      return console.log("Cannot find midjourney channel");
  
    // get input prompt
    const filter = (m) => m.author.id === interaction.user.id;
    interaction.channel.send("Please enter your prompt: ");
  ...
  if (command === "imagine") {
    ...
    if (interaction.channel.name !== "midjourney")
      return console.log("Cannot execute prompt in this channel");
    ...
    // execute prompt
    const response = await interaction.channel.send(prompt);
    response.delete();
  }
};  
